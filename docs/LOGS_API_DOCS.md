...
- ค่าเริ่มต้น `LOG_RETENTION_DAYS=90` อ่านจากไฟล์ `.env` เช่นเดียวกับ DB settings; ปรับได้ตั้งแต่ 1–3650 วัน ค่าผิดทำให้ server ไม่เริ่มทำงาน (ค่า environment ของ process จะไม่ override ค่าใน `.env`)
